...
const ProductMeansureComponent: React.FC<IPropsProductMeansure> = ({
  ...
  return (
    <Box>
      {!showLabel && (
        <InputLabel sx={{ marginTop: "1rem", paddingBottom: "5px" }} required error={!!errors?.product_measure}>
          {t("choose_unit_from_largest")}
        </InputLabel>
      )}
      <Controller
        control={control}
        ...
        render={({ field: { onChange } }) => (
          <Select
            ...
            {...register("product_measure", { required: true })}
            error={!!errors?.product_measure}
      ...
      />
      {errors?.product_measure?.type === "required" && (
        <Typography color="error" p={1}>
          {t("measure_is_required")}
        </Typography>
      )}
    </Box>
  );
