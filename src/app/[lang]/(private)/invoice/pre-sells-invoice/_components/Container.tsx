...
import { useGetPreSellsBillList } from "@/hooks/api/invoice/queries/use-get-pre-sells-bill-list";
import { useDeletePreSellsBillMutation } from "@/hooks/api/invoice/mutations/use-delete-pre-sells-bill";

const PreSalesInvoicePage = () => {
  ...
  const { data, isLoading } = useGetPreSellsBillList({
    page: pageData?.page,
  ...
  const { mutate: deletePreSellsBillMutation, isLoading: deleteIsLoading } =
  useDeletePreSellsBillMutation();

  const handleDeleteFunction = async (event: MouseEvent) => {
    const id = event?.currentTarget?.id;

    deletePreSellsBillMutation(
      { preSellBillId: id },
      {
        onSuccess: ({ message }) => {
          setHandleError({
  ...
  return (
    <InvoiceContextProvider>
      <Box>
        <Typography variant="h3">{t("pre_sell_invoices")}</Typography>
        <Box
          display={"flex"}
...
export default PreSalesInvoicePage;
