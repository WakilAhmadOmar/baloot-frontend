...
import { useGetAllBuyProductReportsQuery } from "@/hooks/api/products-reports/queries/use-get-all-buy-product-reports";

const TotalBuyContainer = () => {
  const [page, setPage] = useState(1);
  const theme = useTheme();
  const t = useTranslations("products_reports");

  const { data: buyProducts, isLoading: isLoadingBuyProducts, refetch } =
  useGetAllBuyProductReportsQuery({
      page: page,
      limit: 10,
    });
  ...
  if (buyProducts?.count === 0 && !isLoadingBuyProducts ){
    return (
  ...
  return (
    <Box>
      ...
      <Box rowGap={"1rem"} columnGap={"1rem"} display={"grid"} pt={2}>
        {buyProducts?.buyProducts.map((product: any) => (
          <ProductItem key={product._id} product={product} />
        ))}
      </Box>
      {buyProducts?.count > 9 && <Box
        display="flex"
      ...
      >
        <Stack spacing={2} p={1}>
          <Pagination
            count={Math.ceil(buyProducts?.count / 10)}
            size={"medium"}
            shape="rounded"
            variant="outlined"
            color="primary"
...
export default TotalBuyContainer;
