...
const DetailsOfAccountPage = ({t}:{t:any}) => {
  const theme = useTheme();
  const router = useRouter();

  const handleBackFunction = () => {
    // router.push({ pathname: "/financialReports/allAccounts" });
  };
  return (
    <Box>
      <Box
      ...
      >
        <ArrowRight2  size={20}/>
        <Typography variant="h3">وکیل احمد عمری</Typography>
      </Box>
      <Box
        display={"flex"}
...
export default DetailsOfAccountPage;
DetailsOfAccountPage.requireAuth = true;
