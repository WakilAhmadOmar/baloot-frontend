import { client } from "@/config/http-client";
import {   GET_PAY_TO_EMPLOYEE_LIST_QUERY_KEY } from "@/constants/queries-key";
import { ClientError } from "@/types";
import { useMutation, UseMutationOptions, useQueryClient } from "react-query";
import { UPDATE_PAY_TO_EMPLOYEE } from "@/graphql/mutation/UPDATE_PAY_TO_EMPLOYEE";



export const useUpdatePayToEmployeeMutation = (options: UseMutationOptions<{}, ClientError, {}> = {}) => {
  const queryClient = useQueryClient()
     const mutationFn = async (variables:any) => {
         const {data: {updateEmployeePayOff } } =  await client.mutate({
          mutation: UPDATE_PAY_TO_EMPLOYEE,
          variables,
        })
        return updateEmployeePayOff
      }
    
    const onSuccess = ()=> {
      queryClient.invalidateQueries({ queryKey: [GET_PAY_TO_EMPLOYEE_LIST_QUERY_KEY] })

    }
    return useMutation({
      mutationFn,
      onSuccess,
      ...options
    })
  };

